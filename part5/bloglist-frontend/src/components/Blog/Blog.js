...
const Blog = (props) => {
  ...
  const addLike = () => {
    ...
    blogService.update(newBlog).then((response) => {
      if (Object.prototype.hasOwnProperty.call(response, "title")) {
        ...
        props.updateMessage({
          content: "Like added and list order maybe modified",
          error: false,
        });
        props.updateList();
      } else {
  ...
  const removeBlog = () => {
    if (window.confirm("Are you sure you want to delete this blog?")) {
      blogService.remove(currentBlog.id).then((response) => {
        if (!Object.prototype.hasOwnProperty.call(response, "error")) {
          ...
          props.updateMessage({
            content: "You don't have the rights to delete this blog.",
            error: true,
          });
        }
      });
